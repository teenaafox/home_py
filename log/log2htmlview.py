...
def open_html_file(file_path):
    try:
        ...
        print(
            f"경고: 'xdg-open' 명령어 실행 중 오류가 발생했습니다. HTML 파일을 수동으로 열어주세요: {file_path}"
        )
    except FileNotFoundError:
        print(
            "경고: 'xdg-open' 명령어를 찾을 수 없습니다. 이 기능은 리눅스 환경에서만 동작합니다."
        )

def main():
    if len(sys.argv) != 4:
        print(
            "사용법: python log2htmlview.py <로그_파일_경로> <읽을_라인_수> <출력_HTML_파일_경로>"
        )
        sys.exit(1)
